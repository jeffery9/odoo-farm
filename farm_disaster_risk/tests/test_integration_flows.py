...
class TestIntegrationFarmDisasterRisk(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.loss.assessment', 'farm.disaster.incident']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_disaster_risk are available """
