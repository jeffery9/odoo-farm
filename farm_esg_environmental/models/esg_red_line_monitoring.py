...
class ESGRedLineConfig(models.Model):
    ...
    _description = 'ESG Red Line Configuration'
...
class ESGRedLineMonitoring(models.Model):
    ...
    _description = 'ESG Red Line Monitoring'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _order = 'detection_date desc'

    name = fields.Char('Monitoring Record', required=True, copy=False)
    red_line_config_id = fields.Many2one('farm.esg.red.line.config', string='Red Line Configuration', required=True)
    ...
    def create(self, vals):
        ...
        if 'name' not in vals or not vals['name']:
            vals['name'] = self.env['ir.sequence'].next_by_code('farm.esg.red.line.monitoring') or '/'
        ...
        for record in self:
            if record.compliance_status in ['warning', 'violation', 'critical'] and not record.alert_issued:
                ...
                self.env['mail.activity'].create({
                    ...
                    'res_id': record.id,
                    'res_model_id': self.env['ir.model'].sudo().search([('model', '=', 'farm.esg.red.line.monitoring')]).id,
    ...
    def action_check_batch_compliance(self, batch_lot_id):
        ...
        config_records = self.env['farm.esg.red.line.config'].search([('active_monitoring', '=', True)])
    ...
    def _cron_check_compliance(self):
        ...
        active_configs = self.env['farm.esg.red.line.config'].search([
...
class StockLot(models.Model):
    ...
    esg_monitoring_ids = fields.One2many('farm.esg.red.line.monitoring', 'batch_lot_id',
    ...
    def action_check_esg_compliance(self):
        ...
        monitoring_model = self.env['farm.esg.red.line.monitoring']
        ...
        return {
            ...
            'res_model': 'farm.esg.red.line.monitoring',
            'view_mode': 'tree,form',
