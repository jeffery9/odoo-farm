...
class TestIntegrationFarmFermentation(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.fermentation.vessel', 'farm.lot.brew', 'farm.fermentation.order']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_fermentation are available """
