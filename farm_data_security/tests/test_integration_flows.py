...
class TestIntegrationFarmDataSecurity(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.data.classification', 'stock.location', 'farm.iot.device.registry', 'res.partner']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_data_security are available """
