...
class TestIntegrationFarmGreenMonitor(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['stock.location', 'project.task', 'report.farm_green_monitor.reduction_trend_report']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_green_monitor are available """
