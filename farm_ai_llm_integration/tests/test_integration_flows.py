...
class TestIntegrationFarmAiLlmIntegration(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['agri.ai.llm.service', 'agri.ai.llm.configuration']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_ai_llm_integration are available """
