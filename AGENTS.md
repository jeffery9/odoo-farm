...
## 2. Multi-Repository Operational Workflow (The Golden Loop)

All agents and developers MUST strictly operate under this three-repo workflow loop to maintain both high velocity and absolute security:

```text
 ┌────────────────────────────────────────────────────────┐
 │ 1. Requirement & Design Phase                          │
 │  - Exclusively inside ../docs/ (Obsidian Vault)        │
 │  - Edit specs, solutions, and Gherkin features         │
 └──────────────────────────┬─────────────────────────────┘
                            │
                            ▼
 ┌────────────────────────────────────────────────────────┐
 │ 2. Implementation & Testing Phase                      │
 │  - Exclusively inside ./ (Private Code Base)           │
 │  - Write TDD Unit/Integration tests -> Implement ORM   │
 │  - Keep docs/ directory completely ignored and clean   │
 └──────────────────────────┬─────────────────────────────┘
                            │
                            ▼
 ┌────────────────────────────────────────────────────────┐
 │ 3. Surgical Release Phase                              │
 │  - Sync code to ../odoo-farm/ via physical override    │
 │  - Run Forensic Leak Check -> Force Push pure code     │
 └────────────────────────────────────────────────────────┘
```

1.  **Specs & Gherkin First**: All requirements, specs, and Gherkin features live in `../docs/` (independent).
2.  **Pure-Code R&D**: All implementation and test verification live in `./` (this private repo).
3.  **Pure-Code Open Source**: All public releases live in `../odoo-farm/` (public, doc-free).
...
## 4. Branching Strategy & Release Flow
...
## 6. Technical Mandates (Odoo 19)

### 6.1 Three-Tier Testing Architecture
...
## 8. Git Worktree Workflow (Multi-Context Paradigm)
...
### 8.2 Operational Guidelines for Agents
...
## 9. STRICT SECURITY & PUBLICATION PROTOCOL (Option A - Decentralized Docs)
