...
class TestIntegrationFarmPlanning(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['agri.intervention.template.input', 'agri.intervention.template', 'agri.technical.route', 'agri.scenario.input.forecast', 'agri.scenario', 'agri.technical.route.line']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_planning are available """
