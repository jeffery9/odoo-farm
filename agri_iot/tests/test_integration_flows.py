...
class TestIntegrationAgriIot(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['iiot.device', 'iiot.telemetry.rule', 'iiot.update', 'iiot.firmware', 'iiot.device.profile', 'iiot.telemetry']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by agri_iot are available """
