...
class TestIntegrationPrecisionProductionIot(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['precision.recipe.phase', 'precision.recipe.control', 'precision.iot.reading', 'precision.iot.device', 'precision.iot.command.log', 'precision.iot.sensor']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by precision_production_iot are available """
