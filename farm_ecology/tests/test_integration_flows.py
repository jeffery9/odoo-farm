...
class TestIntegrationFarmEcology(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.ecological.activity', 'agri.sustainability.biodiversity.indicator', 'agri.sustainability.ecological.zone', 'agri.ecological.activity']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_ecology are available """
