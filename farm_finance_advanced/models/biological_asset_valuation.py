...
class BiologicalAssetValuation(models.Model):
    """
    Biological Asset Valuation and Depreciation - moved from farm_core for better responsibility alignment
...
class BiologicalAssetStageCoefficient(models.Model):
    """
    Growth stage coefficient configuration - moved from farm_core for better responsibility alignment
...
class BiologicalAsset(models.Model):
    """
    Biological Asset Financial Extensions - adds financial functionality to the core biological asset
    US-01-07: Biological Asset Valuation and Depreciation
    NOTE: Core biological asset fields are in farm_core module
    """
    _inherit = 'farm.biological.asset'  # Inherit from the core model instead of duplicating

    # Financial valuation integration - extending the core model
    valuation_ids = fields.One2many('farm.biological.asset.valuation', 'asset_id', string="Asset Valuations")

    # Override the compute method for current_valuation to include financial valuations
    @api.depends('valuation_ids.net_book_value')  # Add dependency on financial valuations
    def _compute_current_valuation(self):
        """Override core method to prioritize financial valuations when available"""
        for asset in self:
            # Check if there are active financial valuations
            active_valuation = asset.valuation_ids.filtered(lambda v: v.state == 'active')
            if active_valuation:
                # Use financial valuation if available
                asset.current_valuation = active_valuation[0].net_book_value
            else:
                # Otherwise, use the original growth-stage-based approach from core module
                stage_multipliers = {
                    'newborn': 0.2,
                    'growing': 0.6,
                    'mature': 1.0,
