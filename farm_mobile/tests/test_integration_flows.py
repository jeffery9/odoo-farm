...
class TestIntegrationFarmMobile(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['agri.evidence', 'mrp.production', 'farm.checklist.submission', 'farm.sync.queue', 'farm.evidence', 'farm.checkin', 'farm.checklist.submission.line', 'farm.expert.call']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_mobile are available """
