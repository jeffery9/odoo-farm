...
class TestIntegrationFarmAgritourism(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.resource', 'sale.order', 'farm.agritourism.operation', 'product.template', 'farm.booking', 'farm.resource.usage', 'sale.order.line']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_agritourism are available """
