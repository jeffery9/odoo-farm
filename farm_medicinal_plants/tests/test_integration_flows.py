...
class TestIntegrationFarmMedicinalPlants(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.medicinal.production', 'farm.medicinal.recipe', 'farm.lot.medicinal']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_medicinal_plants are available """
