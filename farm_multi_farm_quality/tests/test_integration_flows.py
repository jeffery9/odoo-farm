...
class TestIntegrationFarmMultiFarmQuality(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['product.certification', 'cooperative.entity', 'quality.control.standard']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_multi_farm_quality are available """
