...
class AgriDigitalTwinScene(models.Model):
    _name = 'agri.digital.twin.scene'
    _description = 'Agricultural Digital Twin 3D Scene'
    ...
    device_marker_ids = fields.One2many('agri.digital.twin.marker', 'scene_id', string="Device Markers")

    state = fields.Selection([('draft', 'Draft'), ('active', 'Active')], default='draft')

class AgriDigitalTwinMarker(models.Model):
    _name = 'agri.digital.twin.marker'
    _description = 'Agricultural Digital Twin Device Marker'

    scene_id = fields.Many2one('agri.digital.twin.scene', ondelete='cascade')
