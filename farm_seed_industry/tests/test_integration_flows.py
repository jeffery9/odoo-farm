...
class TestIntegrationFarmSeedIndustry(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.seed.recipe', 'farm.seed.production', 'farm.seed.batch']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_seed_industry are available """
