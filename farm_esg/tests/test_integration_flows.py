...
class TestIntegrationFarmEsg(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['esg.framework', 'esg.indicator', 'esg.assessment.line', 'esg.target', 'esg.assessment', 'esg.performance.report']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_esg are available """
