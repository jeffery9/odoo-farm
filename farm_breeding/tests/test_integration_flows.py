...
class TestIntegrationFarmBreeding(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.trait.value', 'farm.breeding.order', 'stock.lot', 'farm.nursery.batch']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_breeding are available """
