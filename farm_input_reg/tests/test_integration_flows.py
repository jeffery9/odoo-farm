...
class TestIntegrationFarmInputReg(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.input.registration.lookup.wizard', 'product.template', 'farm.input.registration.database', 'mrp.production']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_input_reg are available """
