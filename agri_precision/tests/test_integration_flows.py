...
class TestIntegrationAgriPrecisionCore(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['mrp.production', 'agri.precision.mixin', 'stock.lot', 'precision.recipe.phase']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by agri_precision_core are available """
