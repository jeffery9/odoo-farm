...
class TestIntegrationFarmInsurance(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.yield.prediction', 'farm.crop.yield.insurance.policy', 'farm.insurance.claim']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_insurance are available """
