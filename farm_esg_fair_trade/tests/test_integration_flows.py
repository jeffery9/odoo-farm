...
class TestIntegrationFarmEsgFairTrade(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.esg.labor.condition', 'farm.esg.fair.trade.certificate', 'farm.esg.community.investment', 'farm.esg.fair.trade.premium.allocation']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_esg_fair_trade are available """
