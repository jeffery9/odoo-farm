...
class TestIntegrationFarmCrisis(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['stock.lot', 'farm.emergency.protocol', 'farm.crisis.incident', 'sale.order']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_crisis are available """
