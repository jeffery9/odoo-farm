...
class TestIntegrationFarmSubsidy(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.subsidy.application', 'farm.subsidy.program']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_subsidy are available """
