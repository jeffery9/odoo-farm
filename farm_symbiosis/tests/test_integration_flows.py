...
class TestIntegrationFarmSymbiosis(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.symbiotic.plot', 'farm.symbiotic.order', 'farm.lot.rice', 'farm.symbiotic.recipe']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_symbiosis are available """
