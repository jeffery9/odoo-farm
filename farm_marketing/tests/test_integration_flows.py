...
class TestIntegrationFarmMarketing(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['sale.order', 'agri.gi.registry', 'farm.market.connector', 'farm.consumer.feedback', 'farm.market.demand', 'stock.lot', 'res.partner', 'sale.order.line']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_marketing are available """
