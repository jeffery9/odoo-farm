...
## 5. 产销撮合与合规存证 (Market Matching & Compliance)
### 5.1 渠道需求画像匹配
- **需求画像**：捕获 B 端买家的动态需求（如“需要农残检测合格的优质茶叶 500kg”）。
- **实时穿透**：买家在撮合界面可直接穿透查看匹配批次的“合规审计报告”预览版，建立即时交易信任。

### 5.2 跨境准入数字合规证明 (Digital Phyto-Pass)
- **合规存证**：基于 `farm_esg_compliance` 的审计结果，自动生成符合 GlobalGAP 等标准的数字证明摘要。
- **证书验证**：在溯源页面增加“合规背书”卡片，链接到第三方审计 body 的验证页面或系统内置的详尽审计日志。
