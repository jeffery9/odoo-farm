...
## 3. Odoo 开发范式与继承规范
- **Addon 前缀**: 所有自定义模块必须以 `agri_` 或 `farm_` 开头。
- **Model 前缀 (Agri vs. Farm)**:
    - **领域标准 (Domain/Mixin)**: 跨行业的底层物理协议与 Mixin **优先使用 `agri.*`** 前缀（例如：`agri.nutrient.balance`）。
    - **业务实体 (Farm/Application)**: 特定的业务逻辑与操作模型 **必须使用 `farm.`** 前缀（例如：`farm.location.grid`）。
...
### 4.2 Shell 自动化指令
- **高频组合**: 鼓励使用 `&&` 将相关的检查命令组合（如 `git status && git diff HEAD`），以降低 AI 上下文消耗。
- **大规模搜索**: 优先使用优化过的 `grep_search`。

---

### 4.3 无损更新原则 (Lossless Update)
...
## 6. 协作与交付协议 (The Handshake)

### 6.1 任务开始 (On Task Start)
- 必须读取 `task_plan.md` 和 `progress.md`。
- 必须确认当前处理的 User Story 及其验收标准 (AC)。

### 6.2 进度汇报 (Progress Sync)
...
---
**核心提醒**: 记住，我们是在构建一个科学系统。如果一个算法没有在 `docs/algorithms/` 目录下的相关规范文件中备案，它就不应该存在。

ref.  `DEVELOPMENT_CONVENTIONS.md`

---

## 7. 无损修改与审计律令 (Lossless Mandate V1.1)
- **绝对准则**: 严禁自主简化、精简或合并现有逻辑。
- **物理锚点**: [ISA-88], [LOSSLESS], [US-XXX] 标记严禁移除。
- **读-改-核闭环**: 修改前必读全文，修改后必做 git diff 校验。

*V1.1 - Full Restoration & Mandate Alignment | 2026-02-01*
