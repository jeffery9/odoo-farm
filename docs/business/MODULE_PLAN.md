...
| **科学** | `farm_agri_science` | **EPIC 78**: 高级 VRA 算法、生理权重矩阵、生物量亏缺公式增强、RUE/LAI 驱动校准。 | ✅ 完成 (2026-02-02) |
...
## 4. 核心架构模式 (Core Architectural Patterns)

### 4.1 三层物联控制架构 (Three-Tier IoT Architecture)
本项目采用三层解耦架构，通过职责分离确保系统的稳定性与专业性：

| 层级 | 模块名称 | 核心职责 | 依赖关系 |
| :--- | :--- | :--- | :--- |
| **通讯框架 (Framework)** | `agri_iot` | **底层 I/O 通道**：负责物理连接、MQTT 协议、报文编解码及设备注册。 | 无 |
| **管理中心 (Center)** | `farm_iot` | **全局配置中心**：负责资产台账、数字孪生定义及全局映射。记录管理审计日志。 | `agri_iot` |
| **配方边缘 (Edge)** | `precision_production_iot` | **配方边缘执行**：专门针对 ISA-88 生产配方。记录生产专用审计日志。 | `agri_iot`, `precision_production` |

**注**：管理中心与配方边缘物理隔离，数据各自保存，互不依赖。它们均通过 `agri_iot` 与硬件通讯。

### 4.2 插件化 UX 与智能交互 (Pluggable UX & AI Interaction)
系统采用“外观剥离”策略，通过插件化模块实现行业化表现：

- **`farm_ux` (基础框架/术语适配器)**: 
    - **职责**: 作为地基模块，提供 `agri.view.mixin`。它负责全局的术语转换（如将 Odoo 原生的 "MO" 映射为农业语境下的 "Intervention"）。
    - **特性**: 插件化设计，启用后自动应用农业行业视觉风格与术语规范。
- **`farm_ai_decision` (智能交互插件)**: 
    - **职责**: 作为业务增强插件。当同时启用 `farm_operation` 和 `farm_ai_decision` 时，它会通过视图继承将“AI 决策驾驶舱”注入到作业流程中。
    - **特性**: 即插即用，不影响核心作业逻辑。

### 4.2 注入式表现层 (Injection UX Pattern)
系统采用“非侵入式注入”实现去工业化，确保业务逻辑的纯净性：

- **`farm_ux` (UI 注入器)**: 
    - **模式**: **主动注入 (Active Injection)**。该模块通过 `_inherit` Odoo 核心模型，将 `agri.view.mixin` 强制注入到运行栈中。
    - **职责**: 动态拦截视图架构（XML Arch），实时执行术语转换（MO -> Intervention）。
    - **插拔性**: 业务模块（如 `farm_operation`）无需物理依赖 `farm_ux`。仅在 `farm_ux` 被安装时，界面才会发生农业化转变。

**最后更新**: 2026-02-02 (V3.3)
