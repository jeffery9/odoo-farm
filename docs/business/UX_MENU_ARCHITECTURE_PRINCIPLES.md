...
## 5. 主数据分散化原则 (Distributed Master Data Principle)

*   **反模式 (Anti-Pattern)**: 
    *   ❌ 设立一个全局的“基础数据 (Master Data)”或“配置 (Configuration)”顶级 App，把所有的地块、作物品种、农机设备、传感探头全塞进去。
    *   **弊端**: 拖拉机手为了录入一台新农机，要在混合着“猪苗品种”和“温室大棚”的庞大菜单里寻找“农机列表”，严重违背角色的上下文内聚性。
*   **最佳实践 (Best Practice)**: 
    *   ✅ **主数据随业务走**：如果数据是某个 App 执行任务的核心要素，那么该数据就在该 App 内维护。
    *   ✅ *举例*：
        *   【种植 App】: 菜单内包含“地块管理”、“作物品种”、“生长基点配置”。
        *   【IoT App】: 菜单内包含“网关配置”、“传感器档案”、“报警阈值”。
        *   【装备 App】: 菜单内包含“农机档案”、“维保保养周期”。
    *   ✅ **多维投影 (Multi-Projection)**: 如果同一个主数据（如：员工/联系人）在多个 App 都会高频使用，可以通过 `ir.actions.act_window` 在这些 App 的配置菜单中各自挂载一个入口，而不是逼迫用户跳出当前 App 去寻址。
