...
3. **[US-118-03] 极速冻结中心温度监控 (Flash Freezing Control)**：💡 待实现
    - **描述**：作为车间品控，我希望监控速冻机的运行曲线，确保产品中心温度在规定时间内降至 -18℃。
    - **验收条件**：
        - **(IoT)** 接收速冻机温度探头数据。
        - **(Gate)** 继承 `AgriQualityGateMixin`，若速冻曲线不合格，拦截批次入库。

4. **[US-118-04] 微生物指标与出口核验 (Microbial Gate)**：💡 待实现
