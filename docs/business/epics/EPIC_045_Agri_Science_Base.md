# EPIC 045:农学科学底座 (Agri-Science Foundation)
...
- **验收标准 (AC)**:
    ...
    *   **验收标准 (Acceptance Criteria)**:
        *   **AC1 (界面与交互)**: 用户界面必须清晰展示【品种生理指纹与基准 (Physiology Fingerprint)】的核心字段和操作按钮，且响应式兼容移动端访问。
...
- **验收标准 (AC)**:
    ...
    *   **验收标准 (Acceptance Criteria)**:
        *   **AC1 (界面与交互)**: 用户界面必须清晰展示【生理阶段与积温驱动 (Physiological Stages & GDD)】的核心字段和操作按钮，且响应式兼容移动端访问。
        *   **AC2 (业务流转)**: 核心状态机（State Machine）的流转必须准确无误，确保模块间集成的边界数据一致性。
        *   **AC3 (权限控制)**: 只有具备对应权限组（如 Specialist/Manager）的角色才能执行该业务的写操作或状态确认。
...
- **验收标准 (AC)**:
    ...
    *   **验收标准 (Acceptance Criteria)**:
        *   **AC1 (界面与交互)**: 用户界面必须清晰展示【环境阈值矩阵与 SPC 联动 (Environmental Matrix)】的核心字段和操作按钮，且响应式兼容移动端访问。
        *   **AC2 (业务流转)**: 核心状态机（State Machine）的流转必须准确无误，确保模块间集成的边界数据一致性。
        *   **AC3 (权限控制)**: 只有具备对应权限组（如 Specialist/Manager）的角色才能执行该业务的写操作或状态确认。
...
- **验收标准 (AC)**:
    ...
    *   **验收标准 (Acceptance Criteria)**:
        *   **AC1 (界面与交互)**: 用户界面必须清晰展示【养分吸收动态模型 (Nutrient Uptake Curves)】的核心字段和操作按钮，且响应式兼容移动端访问。
        *   **AC2 (业务流转)**: 核心状态机（State Machine）的流转必须准确无误，确保模块间集成的边界数据一致性。
        *   **AC3 (权限控制)**: 只有具备对应权限组（如 Specialist/Manager）的角色才能执行该业务的写操作或状态确认。


### **[US-045-05] 科学向导：配方自校验 (Scientific Recipe Validator)**
...
---
    *   **验收标准 (Acceptance Criteria)**:
        *   **AC1 (界面与交互)**: 用户界面必须清晰展示【科学向导：配方自校验 (Scientific Recipe Validator)】的核心字段和操作按钮，且响应式兼容移动端访问。
        *   **AC2 (业务流转)**: 核心状态机（State Machine）的流转必须准确无误，确保模块间集成的边界数据一致性。
        *   **AC3 (权限控制)**: 只有具备对应权限组（如 Specialist/Manager）的角色才能执行该业务的写操作或状态确认。
...
- **验收标准 (AC)**:
    ...
    *   **验收标准 (Acceptance Criteria)**:
        *   **AC1 (界面与交互)**: 用户界面必须清晰展示【视觉物候标定与反馈 (Vision-Phenology Sync)】的核心字段和操作按钮，且响应式兼容移动端访问。
        *   **AC2 (业务流转)**: 核心状态机（State Machine）的流转必须准确无误，确保模块间集成的边界数据一致性。
        *   **AC3 (权限控制)**: 只有具备对应权限组（如 Specialist/Manager）的角色才能执行该业务的写操作或状态确认。
...
- **验收标准 (AC)**:
    ...
    *   **验收标准 (Acceptance Criteria)**:
        *   **AC1 (界面与交互)**: 用户界面必须清晰展示【生物压力累积评估 (Biological Stress Analysis)】的核心字段和操作按钮，且响应式兼容移动端访问。
        *   **AC2 (业务流转)**: 核心状态机（State Machine）的流转必须准确无误，确保模块间集成的边界数据一致性。
        *   **AC3 (权限控制)**: 只有具备对应权限组（如 Specialist/Manager）的角色才能执行该业务的写操作或状态确认。
...
- **验收标准 (AC)**:
    ...
    *   **验收标准 (Acceptance Criteria)**:
        *   **AC1 (界面与交互)**: 用户界面必须清晰展示【生物资源转化效率 (Biological Conversion Efficiency)】的核心字段和操作按钮，且响应式兼容移动端访问。
        *   **AC2 (业务流转)**: 核心状态机（State Machine）的流转必须准确无误，确保模块间集成的边界数据一致性。
        *   **AC3 (权限控制)**: 只有具备对应权限组（如 Specialist/Manager）的角色才能执行该业务的写操作或状态确认。
...
- **验收标准 (AC)**:
    ...
    *   **验收标准 (Acceptance Criteria)**:
        *   **AC1 (界面与交互)**: 用户界面必须清晰展示【科学性能看板 (Scientific Performance Dashboard)】的核心字段和操作按钮，且响应式兼容移动端访问。
        *   **AC2 (业务流转)**: 核心状态机（State Machine）的流转必须准确无误，确保模块间集成的边界数据一致性。
        *   **AC3 (权限控制)**: 只有具备对应权限组（如 Specialist/Manager）的角色才能执行该业务的写操作或状态确认。
