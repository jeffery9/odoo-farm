...
class TestIntegrationFarmViticulture(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.viticulture.cycle', 'farm.lot.grape', 'farm.viticulture.plot']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_viticulture are available """
