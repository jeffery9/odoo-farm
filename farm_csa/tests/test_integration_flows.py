...
class TestIntegrationFarmCsa(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.csa.plan', 'farm.shared.tool', 'farm.csa.subscription']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_csa are available """
