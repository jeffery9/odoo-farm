...
class TestIntegrationFarmSupplyCore(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['supply.common.fields.mixin', 'agri.supply.chain.node.mixin', 'supply.chain.node.mixin']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_supply_core are available """
