...
class TestIntegrationFarmMultiFarmEquipment(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['shared.machinery.pool', 'machinery.rental', 'cooperative.entity', 'cooperative.member']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_multi_farm_equipment are available """
