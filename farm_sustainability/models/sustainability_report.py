...
class SustainabilityDashboard(models.Model):
    ...
    def _compute_scores(self):
        ...
        for dashboard in self:
            # 获取该期间内的指标数据
            economic_metrics = self.env['farm.sustainability.metric'].search([
                ('category', '=', 'economic'),
                ('is_active', '=', True)
            ])

            environmental_metrics = self.env['farm.sustainability.metric'].search([
                ('category', '=', 'environmental'),
                ('is_active', '=', True)
            ])

            social_metrics = self.env['farm.sustainability.metric'].search([
                ('category', '=', 'social'),
                ('is_active', '=', True)
            ])
    ...
    def _compute_kpis(self):
        ...
        for dashboard in self:
            # 统计循环经济流程
            circular_flows = self.env['farm.sustainability.circular.flow'].search([
            ...
            # 集成碳足迹数据
            carbon_calculations = self.env['farm.sustainability.carbon.footprint.calculation'].search([
    ...
    def _calculate_period_score(self, category, start_date, end_date):
        """计算指定期间和类别的得分"""
        metrics = self.env['farm.sustainability.metric'].search([
            ('category', '=', category),
            ('is_active', '=', True)
        ])
        if not metrics:
    ...
    def action_view_circular_flows(self):
        ...
        return {
            'type': 'ir.actions.act_window',
            'name': _('循环经济流程'),
            'res_model': 'farm.sustainability.circular.flow',
            'view_mode': 'tree,form',
    ...
    def action_view_carbon_footprints(self):
        ...
        return {
            'type': 'ir.actions.act_window',
            'name': _('碳足迹数据'),
            'res_model': 'farm.sustainability.carbon.footprint.calculation',
            'view_mode': 'tree,form',
            'context': self.env.context
        }
...
class SustainabilityReport(models.Model):
    ...
    def create_report(self, period_start, period_end):
        """自动生成可持续发展报告"""
        # 计算各维度得分
        economic_metrics = self.env['farm.sustainability.metric'].search([('category', '=', 'economic'), ('is_active', '=', True)])
        environmental_metrics = self.env['farm.sustainability.metric'].search([('category', '=', 'environmental'), ('is_active', '=', True)])
        social_metrics = self.env['farm.sustainability.metric'].search([('category', '=', 'social'), ('is_active', '=', True)])
