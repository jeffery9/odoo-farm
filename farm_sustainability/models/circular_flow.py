...
class CircularFlow(models.Model):
    ...
    _description = 'Farm Circular Flow'
    ...
    # 集成现有碳足迹计算
    related_carbon_calculation_id = fields.Many2one('farm.sustainability.carbon.footprint.calculation', '关联碳足迹计算')
...
class CircularFlowAnalysis(models.Model):
    ...
    _description = 'Farm Circular Flow Analysis'
    _auto = False  # 这是一个视图模型，不创建实际表

    flow_id = fields.Many2one('farm.sustainability.circular.flow', '循环经济流', readonly=True)
    ...
    def init(self):
        ...
        self.env.cr.execute("""
            CREATE OR REPLACE VIEW %s AS (
                ...
                FROM farm_sustainability_circular_flow cf
