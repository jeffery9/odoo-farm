...
class TestISLCore(TransactionCase):
    ...
    def test_10_isl_navigation_action(self):
        ...
        self.assertEqual(action['res_model'], 'farm.livestock.production')
        ...
        self.assertEqual(bom_action['res_model'], 'farm.livestock.bom')
    ...
    def test_12_lot_isl_record_type_computation(self):
        ...
        isl_lot2 = self.env['farm.lot.aquaculture'].create({
            'lot_id': lot2.id,
            'current_count': 50
        })
    ...
    def test_13_lot_isl_navigation_action(self):
        ...
        # Verify the action returns correct view
        self.assertEqual(action['type'], 'ir.actions.act_window')
        self.assertEqual(action['res_model'], 'farm.lot.livestock')
    ...
    def test_15_auto_isl_creation_on_bom_line(self):
        ...
        isl_bom_line = self.env['farm.livestock.bom.line'].search([('bom_line_id', '=', bom_line.id)])
    ...
    def test_19_bom_line_isl_navigation_action(self):
        ...
        # Verify the action returns correct view
        self.assertEqual(action['type'], 'ir.actions.act_window')
        self.assertEqual(action['res_model'], 'farm.livestock.bom.line')
        self.assertEqual(action['view_mode'], 'form')
