...
class TestIntegrationFarmSaleCh(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['export.compliance.log', 'export.certificate', 'export.country.standard', 'sale.order.line']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_sale_ch are available """
