...
class ESGComplianceMonitoring(models.Model):
    ...
    _description = 'ESG Compliance Monitoring'
    ...
    ], string='Implementation Status', default='pending')
        ...
        for record in self:
            if record.compliance_status in ['warning', 'non_compliant', 'critical']:
                ...
                self.env['mail.activity'].create({
                    ...
                    'res_id': record.id,
                    'res_model_id': self.env['ir.model'].sudo().search([('model', '=', 'farm.esg.compliance.monitoring')]).id,
