...
class TestIntegrationFarmEntityReg(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['res.company', 'farm.cooperative.member']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_entity_reg are available """
