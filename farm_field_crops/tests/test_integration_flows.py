...
class TestIntegrationFarmFieldCrops(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['mrp.production', 'farm.crop.lot', 'farm.crop.production', 'farm.crop.bom.line', 'farm.field.crop.operation', 'stock.lot', 'mrp.bom', 'farm.crop.bom']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_field_crops are available """
