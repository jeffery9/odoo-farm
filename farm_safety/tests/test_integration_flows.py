...
class TestIntegrationFarmSafety(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.prevention.line', 'project.task', 'farm.prevention.template', 'farm.biosafety.access.log', 'stock.lot', 'stock.picking']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_safety are available """
