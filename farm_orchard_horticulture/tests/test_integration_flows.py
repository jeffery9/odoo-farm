...
class TestIntegrationFarmOrchardHorticulture(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.orchard.cycle', 'stock.lot', 'farm.orchard.operation']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_orchard_horticulture are available """
