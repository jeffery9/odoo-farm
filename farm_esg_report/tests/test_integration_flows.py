...
class TestIntegrationFarmEsgReport(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.esg.dashboard', 'farm.esg.report', 'farm.esg.sustainability.goal']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_esg_report are available """
