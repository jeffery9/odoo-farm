...
class TestIntegrationFarmFinancialValuation(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.financial.valuation.template', 'farm.financial.asset.valuation']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_financial_valuation are available """
