...
class TestIntegrationFarmHr(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['farm.employee.certificate', 'farm.worklog', 'project.task', 'farm.agri.skill', 'hr.employee', 'farm.wage.rule', 'farm.labor.payment', 'farm.labor.payment.line']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_hr are available """
