...
class TestIntegrationFarmFloriculture(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.models_to_check = ['sale.order', 'farm.flower.order', 'farm.flower.bom', 'farm.lot.flower', 'sale.order.line']

    def test_01_cross_model_dependencies(self):
        """ Verify that dependencies and XML IDs required by farm_floriculture are available """
